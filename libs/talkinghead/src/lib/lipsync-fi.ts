...
class LipsyncFi {
  private visemes: { [key: string]: string };
   ...
   */
  numberToFinnishWords(x: string) {
    const w = [];
    let n = parseFloat(x);
    if (n === undefined) return x;
    let p = (n: number, z: number, w0: string, w1: string, w2: string) => {
      if (n < z) return n;
      const d = Math.floor(n / z);
      w.push(
        w0 + (d === 1 ? w1 : this.numberToFinnishWords(d.toString()) + w2),
      );
      return n - d * z;
    };
    if (n < 0) {
    ...
    if (n > 0) {
      ...
      if (d.length > 1) {
        ...
        for (let i = 0; i < c.length; i++) {
          w.push(' ' + this.numbers[c[i] as any]);
        }
      }
    }
    return w.join('').trim();
  ...
  preProcessText(s: {
    ...
    return s
      .replace(/[#_*\'\":;]/g, '')
      .replace(this.symbolsReg, (symbol: string | number) => {
      ...
      .normalize('NFD')
      .replace(/[\u0300-\u0307\u0309\u030b-\u036f]/g, '')
      .normalize('NFC') // Remove non-Finnish diacritics
      .trim();
  }
